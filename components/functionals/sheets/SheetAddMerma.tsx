...
import { addMerma } from '@/app/(with-layout)/merma/actions';

export default function SheetAddMerma({
  productos,
}: {
  ...
  const form = useForm<InferInput<typeof MermaCafeteriaSchema>>({
    resolver: valibotResolver(MermaCafeteriaSchema),
    defaultValues: {
  ...
  const onSubmit = async (
    dataForm: InferInput<typeof MermaCafeteriaSchema>
  ): Promise<void> => {
    const { data: dataRes, error } = await addMerma(dataForm);
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen} modal={true}>
      ...
      <SheetContent className="w-full sm:max-w-[600px] overflow-y-scroll">
        <SheetHeader>
          <SheetTitle>Agregar merma</SheetTitle>
          <SheetDescription className="pb-4">
            Rellene el formulario para agregar una merma.
          </SheetDescription>
          {error && (
