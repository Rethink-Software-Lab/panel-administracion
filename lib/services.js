...
export async function getProducts(variables) {
  const result = await fecthBase(ALL_PRODUCTS_INFO, variables);
  return {
    errors: result?.errors || null,
    data: result?.data?.allProductosInfo || null,
  };
}
...
export async function inventarioAlmacen(variables) {
  const result = await fecthBase(INVENTARIO_ALMACEN, variables);
  return {
    errors: result?.errors || null,
    data: result?.data?.inventarioAlmacen || null,
  };
}
