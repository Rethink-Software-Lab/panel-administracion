...
export const inventarioProductoinfo = pgTable(
  ...
  (table) => [
    ...
    index('inventario_productoinfo_codigo_d029faf9_like').using(
      'btree',
      table.codigo.asc().nullsLast().op('varchar_pattern_ops')
    ),
...
export const inventarioTransacciones = pgTable(
  ...
  (table) => [
    ...
    unique('inventario_transferenciastarjetas_venta_id_35c04c04_uniq').on(
      table.ventaId
    ),
    unique('inventario_transferenciastarjetas_venta_cafeteria_id_key').on(
      table.ventaCafeteriaId
    ),
  ]
);
